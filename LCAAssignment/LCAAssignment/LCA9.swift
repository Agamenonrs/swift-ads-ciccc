...
//                    }
                ...
                if start <= p && start <= q && auxFoundP && auxFoundQ {
                    if !array.contains(start){
                        array.append(start)
                        print(start)
                    }
    ...
    var array: [Int] = []
    let edgesToCheckList = [[6,11]]
    for pair in edgesToCheckList {
        var visited = [Bool](repeating: false, count: n + 1)
        var foundP = false
        var foundQ = false
        array = []
        dfs(start: 1, adjList: &adjList, visited: &visited,p: pair[0],q:pair[1], &foundP, &foundQ)
//        if pair[0] < pair[1] {
//            if !array.contains(pair[0]) {
//                array.append(pair[0])
//            }
//            if !array.contains(pair[1]) {
//                array.append(pair[1])
//            }
//        }else{
//            if !array.contains(pair[1]) {
